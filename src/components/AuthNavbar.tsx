'use client'; // On précise que ce code est interactif (il réagit aux clics de l'utilisateur)

import Link from 'next/link';
import { usePathname } from 'next/navigation'; // Le GPS de Next.js qui détecte sur quelle page on se trouve
import Cookies from 'js-cookie'; // Notre portefeuille pour vérifier le Badge VIP (le token)
import { useEffect, useState } from 'react';
import Image from 'next/image';

//Fabrique les initiales pour l'avatar
const getInitials = (name: string | null) => {
    if (!name) return '??'; // Sécurité : si pas de nom, on affiche "??"
    const parts = name.trim().split(' '); // On coupe le texte là où il y a un espace
    if (parts.length >= 2) {
        return (parts[0][0] + parts[1][0]).toUpperCase(); // Prend la 1ère lettre du mot 1 et du mot 2
    }
    return name.substring(0, 2).toUpperCase(); // Si un seul mot, prend les 2 premières lettres
};
...
export default function AuthNavbar() {
    ...
    // Si on n'a pas le badge VIP, on ne montre pas la navbar non plus
    if (!isAuthenticated) return null;

    // Vérifie si le bouton correspond à la page où on est
    ...
    return (
        <nav className="w-full h-[94px] bg-[#FFFFFF] border-b border-gray-100 flex items-center justify-between px-[100px] relative font-sans">
            ...
            {/* 3. PARTIE DROITE : L'Avatar utilisateur */}
            <div className="flex-shrink-0 flex justify-end">
                {/* On enveloppe l'avatar dans un lien pour aller sur la page Profil */}
                <Link href="/profile" className="flex items-center justify-center w-[65px] h-[65px] bg-[#FFE8D9] rounded-full hover:ring-2 ring-orange-300 transition-all cursor-pointer">
                    <span
                        className="text-[#0F0F0F] text-[14px] font-normal tracking-wide"
                        style={{ fontFamily: "'Inter', sans-serif" }}
                    >
                        {/* On appelle notre fonction outil pour afficher "AD" */}
